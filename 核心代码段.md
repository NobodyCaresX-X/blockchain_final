...
def create_project(self, payload: dict[str, Any]) -> dict[str, Any]:
    self._ensure_initialized()
    self.ensure_ready()
    milestone_descriptions = payload.get("milestoneDescriptions", [])
    milestone_thresholds = payload.get("milestoneThresholdsBps", [])
    milestone_release_bps = payload.get("milestoneReleaseBps", [])
...
def _format_project(self, project_id: int) -> ProjectSnapshot:
    ...
    milestones = []
    for index in range(_to_int(data[14])):
        milestone = self.contract.functions.getMilestone(project_id, index).call()
        milestones.append(
            {
                "index": index,
                "description": milestone[0],
                "completionThresholdBps": _to_int(milestone[1]),
                "releaseBps": _to_int(milestone[2]),
                "completed": bool(milestone[3]),
                "released": bool(milestone[4]),
            }
        )
    ...
    return ProjectSnapshot(
        ...
        milestoneCount=_to_int(data[14]),
...
function _calculateProportionalRefund(uint256 projectId) private {
    ...
    for (uint256 i = 0; i < project.milestones.length; i++) {
        ...
        uint256 requiredPledged = (project.goal * milestone.completionThresholdBps) / BPS;
        
        if (project.pledged >= requiredPledged) {
            uint256 releaseAmount = (project.goal * milestone.releaseBps) / BPS;
            totalRetained += releaseAmount;
    ...
    project.totalWithdrawn = totalRetained;
}
```
...
function checkAndFinishExpired() external {
    for (uint256 i = 0; i < projectIds.length; i++) {
        ...
        if (!project.ended && block.timestamp >= project.deadline) {
            ...
            emit ProjectFinished(projectId, project.status);
        }
    }
}
```
